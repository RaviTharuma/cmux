...
private actor CmuxConfigActionCatalogReadCoordinator {
    enum Lane: Sendable {
        case global
        case general
    }
    ...
    private struct ActiveRead {
        ...
        let requestID: UUID
        let lane: Lane
        let operation: @Sendable () -> CmuxConfigActionCatalogSource
        var isRunning: Bool
    ...
    private static let maximumPendingReadCount = 64
    private nonisolated static let ioQueue = DispatchQueue(
        label: "com.cmux.action-catalog-io",
        qos: .utility,
    ...
    func run(
        ...
        lane: Lane,
        requestID: UUID,
        operation: @escaping @Sendable () -> CmuxConfigActionCatalogSource
    ...
    private func waitForRead(
        ...
        return await withTaskCancellationHandler {
            await withCheckedContinuation { continuation in
                ...
                if activeReads[key] == nil {
                    ...
                    guard canStartImmediately
                            || pendingReadKeys.count < Self.maximumPendingReadCount else {
                        continuation.resume(returning: .cancelled)
                        return
                    }
            ...
            // This actor cannot service cancellation until the synchronous
            // continuation-registration closure above has finished, so a
            // missing waiter is already complete and needs no tombstone.
    ...
    private func startRead(key: String) {
        ...
        Task { [weak self] in
            let source = await Self.performRead(read.operation)
            await self?.finishRead(key: key, readID: read.id, source: source)
        }
    }

    private nonisolated static func performRead(
        _ operation: @escaping @Sendable () -> CmuxConfigActionCatalogSource
    ) async -> CmuxConfigActionCatalogSource {
        await withCheckedContinuation { continuation in
            ioQueue.async {
    ...
    private func finishRead(
        ...
        for waiter in read.waiters.values {
            waiter.continuation.resume(returning: waiter.requestID == read.requestID
                ? .source(source)
                : .retry)
        }
    ...
    private func canStartRead(in lane: Lane) -> Bool {
        switch lane {
        case .global:
            return activeGlobalReadCount < 1
        case .general:
            return activeGeneralReadCount < 2
        }
    }
...
final class CmuxConfigStore: ObservableObject {
    private struct ActionCatalogFreshWaiter {
    ...
    private var actionCatalogFreshWaiters: [String: [UUID: ActionCatalogFreshWaiter]] = [:]
    ...
    nonisolated static func loadActionCatalogSource(
        ...
        let localPath = normalizedDirectory.map { directory in
                ...
                let deadlineTask = deadline.map { deadline in
                    Task { @MainActor [weak self] in
                        while !Task.isCancelled {
                            ...
                            // Short bounded sleeps avoid overflowing nanosecond
                            // conversion for Date.distantFuture or infinity.
                            try? await Task.sleep(for: .seconds(min(remaining, 86_400)))
                        }
    ...
    private func scheduleActionCatalogRefresh(
        ...
        let task: Task<CmuxConfigActionCatalogSnapshot?, Never> = Task { @MainActor [weak self] in
            let source = await CmuxConfigActionCatalogReadCoordinator.shared.run(
