...
public struct BetaFeaturesSection: View {
    ...
    private var remoteHerdrMirrorRow: some View {
        SettingsCardRow(
            ...
            String(localized: "settings.betaFeatures.remoteHerdrMirror", defaultValue: "Mirror tabs like ssh-tmux"),
            subtitle: remoteHerdrMirror.current
                ? String(localized: "settings.betaFeatures.remoteHerdrMirror.subtitleOn", defaultValue: "Mirrors each Herdr tab as a cmux tab with real Bonsplit panes and Ghostty surfaces. Quitting cmux detaches; the Herdr session stays up.")
                : String(localized: "settings.betaFeatures.remoteHerdrMirror.subtitleOff", defaultValue: "Hides the Herdr window mirror until you enable it here. Nested Topology stays the sidebar navigator.")
        ) {
